...
std::vector<std::vector<Document>> ProcessQueries(const SearchServer& search_server, const std::vector<std::string>& queries) {
    ...
    transform(std::execution::par, queries.begin(), queries.end(), document_list.begin(), [&search_server](std::string local_) {return search_server.FindTopDocuments(local_); });
